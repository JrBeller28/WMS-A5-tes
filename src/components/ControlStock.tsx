...
export function ControlStock({ searchQuery = '' }: { searchQuery?: string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const locatorMap = new Map<string, { qtyIn: number; qtyOut: number }>();
      ...
      transactions.forEach((tx: Transaction) => {
        ...
        switch (tx.type) {
          case 'INBOUND': {
            ...
            const current = locatorMap.get(key) || { qtyIn: 0, qtyOut: 0 };
          ...
          case 'TRANSFER': {
            ...
            const outCurrent = locatorMap.get(outKey) || { qtyIn: 0, qtyOut: 0 };
            ...
            if (tx.transferToLocatorId) {
              ...
              const inCurrent = locatorMap.get(inKey) || { qtyIn: 0, qtyOut: 0 };
              ...
              {loading ? (
                ...
                filteredData.map((row, idx) => (
                  <tr key={`${row.sku}-${row.locatorId}-${idx}`} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-3 font-bold text-emerald-700">{row.locatorId}</td>
